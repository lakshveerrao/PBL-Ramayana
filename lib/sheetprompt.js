...
export function viewPrompt(id, slot) {
  ...
  // A declared replacement for one view slot's shot description, matched exactly so a
  // brief that drifts fails loudly instead of silently not being replaced.
  const va = adjustments()._views;
  let shot = view.shot;
  if (va?.slot === slot && va.replaces) {
    if (va.replaces !== view.shot) {
      throw new Error(`direction/anchor-adjustments.json replaces the "${slot}" view text and direction/sheet-briefs.json no longer carries it. The adjustment and the brief have drifted apart.`);
    }
    if (va.class !== 'S' || !va.reason) throw new Error('the view adjustment does not declare class S with a reason');
    shot = va.with;
  }

  const parts = [];
  // A character whose brief withholds the face is not a portrait subject: Kausalya's
  // plate is her hands, and her own never list forbids a portrait. validate caught
  // this the first time the portrait framing went in.
  const framing = c.face_withheld ? null : adjustments()._framing;
  parts.push(framing?.with
    ? framing.with.replace('{shot}', shot)
    : `Character model sheet, ${shot}. 9:16 vertical.`);
