...
// An entity DECLARED in direction/references.json as a place or a crowd has a look and
// no identity. COURT is "the ministers and court": the graph types it as a person, so
// this gate demanded a model sheet for it, and a crowd has no face to lock - it could
// never clear, and it blocked six shots permanently.
//
// The exemption is narrow on purpose. CLAUDE.md's rule stands: an entity the studio
// knows nothing about is treated as a person and refused. Only an entity a human has
// declared, in writing and with a reason, is exempt - and the declaration supplies the
// reference plates that shot is then drawn from, so the court is one court across the
// arc rather than a new room full of strangers each time.
function declaredAsNotAPerson(entityId) {
  let cfg = null;
  try { cfg = JSON.parse(readFileSync(join(ROOT, 'direction', 'references.json'), 'utf8')); } catch { return false; }
  const e = cfg.entities?.[entityId];
  return Boolean(e && (e.kind === 'place' || e.kind === 'crowd') && (e.files ?? []).length > 0);
}

export function checkShot(shot) {
  const all = shot.entities ?? [];
  const entities = all.filter((e) => isPerson(e) && !declaredAsNotAPerson(e));
  const nonPeople = all.filter((e) => !isPerson(e) || declaredAsNotAPerson(e));
