...
import { checkNarration } from './register.js';
...
export function reconcile(out, filmId) {
  ...
  // Shot-level contract, checked here rather than paid for.
  const seen = new Set();
  const index = new Map(shots.map((s, i) => [s.id, i]));
  for (const [i, s] of shots.entries()) {
    if (!s.id) problems.push(`shot ${i} has no id`);
    if (seen.has(s.id)) problems.push(`shot ${s.id} appears twice`);
    seen.add(s.id);
    const ref = s.reuse_of ?? s.crop_of;
    if (ref && !index.has(ref)) problems.push(`shot ${s.id} reuses ${ref}, which does not exist`);
    else if (ref && index.get(ref) >= i) problems.push(`shot ${s.id} reuses ${ref}, which comes later`);
  }

  // The register, offline. The critic costs money; this does not, and catches the
  // same drift deterministically.
  const register = checkNarration(narration);
  for (const f of register.findings) problems.push(`narration ${f.line_id}: ${f.problem}`);

  return { ok: problems.length === 0, problems, repairs, shots, narration, total_frames: runFrames, register };
}
