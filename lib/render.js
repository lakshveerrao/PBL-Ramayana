...
export function estimate(filmId) {
  ...
  const motionShots = Object.entries(read('effects').shots)
  ...
  const chars = Object.values(t.narration).reduce((a, n) => a + n.en.length + n.hi.length + n.te.length, 0);
...
export async function renderShot(filmId, shotId, { allow_spend = null } = {}) {
  ...
  const { prompt, negatives } = assemble(shot, filmId);
  ...
  const out = await providers.image.generate({ prompt, seed: seedFor(filmId, shotId), width: 1080, height: 1920 });
...
export async function renderMotion(filmId, shotId, { allow_spend = null } = {}) {
  ...
  const spec = effects.shots[shotId];
  ...
  return { spent: true, usd: est, shot: shotId, url: out.url, instruction: spec.instruction, criteria: effects.rejection_criteria };
}
