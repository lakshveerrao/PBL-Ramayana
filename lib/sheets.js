...
import { read, write, ensureDir, assetsDir, ROOT } from './store.js';
...
export function uploadSheet(id, { files = [] } = {}) {
  ...
  const dir = ensureDir(`${assetsDir()}/sheets/${id}`);
  const stored = [];
  const hashes = [];
  ...
  for (const f of files) {
    ...
    if (f.data_base64) {
      ...
      // A sheet is read from inside the repo or not at all.
      if (!abs.startsWith(ROOT + sep)) throw new Error(`path for slot ${f.slot} escapes the repository`);
      if (!existsSync(abs)) throw new Error(`path for slot ${f.slot} does not exist: ${f.path}`);
      buf = readFileSync(abs);
    } else {
      throw new Error(`file for slot ${f.slot} has neither data_base64 nor a path`);
    }
    if (buf.length === 0) throw new Error(`file for slot ${f.slot} is empty`);

    const ext = (f.filename ?? 'sheet.png').match(/\.[a-z0-9]+$/i)?.[0] ?? '.png';
    const name = `${f.slot}${ext}`;
    writeFileSync(join(dir, name), buf);
    stored.push(`${assetsDir()}/sheets/${id}/${name}`);
...
// A placeholder is not a name. Three approvals in a row arrived as "[YOUR NAME]" and
// then "NAME" - a template that was never filled in. The old check only measured
// length, so "NAME" would have been written into the permanent record behind every
// frame of the arc as the person who approved it.
//
// This refuses the shapes a template leaves behind. It cannot catch every one, and it
// is not trying to: a human who types a real name is trusted, and the point is that a
// human types it.
const PLACEHOLDER = [
  /^\[.*\]$/,                       // [YOUR NAME], [NAME], [...]
  /^<.*>$/,                          // <your name>
  /^(your[\s_-]*)?name$/i,           // NAME, your name, your_name
  /^full[\s_-]*name$/i,
  /^(approver|director|user|admin|me|self|someone|anon|anonymous)$/i,
  /^(todo|tbd|tba|xxx+|placeholder|insert[\s_-]*name|sign[\s_-]*here|n\/?a)$/i,
  /^-+$/,
];

export function looksLikePlaceholder(who) {
  const w = String(who ?? '').trim();
  return PLACEHOLDER.some((re) => re.test(w));
}
...
export function approveSheet(id, { approver = null, name = null } = {}) {
  ...
  if (looksLikePlaceholder(who)) {
    throw new Error(
      `"${String(who).trim()}" is a placeholder, not a name. This name goes into the permanent record behind every `
      + 'frame the sheet conditions, and the gate exists so that a person puts their own name to it.',
    );
  }
  const sheets = read('sheets', { fresh: true });
  const sheet = sheets.sheets.find((s) => s.id === id);
