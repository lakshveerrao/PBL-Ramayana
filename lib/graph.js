...
import { read, treatment, ROOT } from './store.js';
...
// References for entities that carry a look but not an identity - a hall, a crowd.
//
// Until the court plates arrived, a shot was conditioned only on the SHEETS of the
// people in it, so the hall contributed nothing and COURT blocked the gate forever
// because a crowd cannot have a model sheet.
//
// These are not sheets. The consistency gate binds on people and keeps binding only on
// people: nothing here clears a principal or stands in for one. A reference here
// changes how a shot is DRAWN, never who is allowed in it.
export function entityReferences(entityIds = []) {
  let cfg = null;
  try { cfg = JSON.parse(readFileSync(join(ROOT, 'direction', 'references.json'), 'utf8')); } catch { return []; }
  const out = [];
  for (const id of entityIds) {
    const e = cfg.entities?.[id];
    if (!e) continue;
    for (const f of e.files ?? []) {
      if (!existsSync(join(ROOT, f))) {
        throw new Error(`direction/references.json names ${f} for ${id} and it is not in the repo.`);
      }
      out.push({ entity: id, kind: e.kind, file: f });
    }
  }
  return out;
}

// Which frame a cross-film reuse actually shows.
//
// A treatment only knows its own shots, so a shot reusing a frame from a different film
// has source=reuse and reuse_of=null - eight of them across the arc. HANDOFF §7 names
// the chains; direction/shared-plates.json records them.
export function sharedPlateFor(filmId, shotId) {
  let cfg = null;
  try { cfg = JSON.parse(readFileSync(join(ROOT, 'direction', 'shared-plates.json'), 'utf8')); } catch { return null; }
  for (const p of cfg.plates ?? []) {
    for (const [f, s] of p.shown_in ?? []) {
      if (f === filmId && s === shotId) return { film: p.made_in, shot: p.shot, plate: p.plate };
    }
  }
  return null;
}

// Every shot that names no source of its own, so a check can prove none is missed.
export function unresolvedReuses() {
  const out = [];
  for (const f of read('films').films) {
    let t; try { t = treatment(f.id); } catch { continue; }
    for (const s of t?.shots ?? []) {
      if (s.source === 'generate' || s.reuse_of || s.crop_of) continue;
      out.push({ film: f.id, shot: s.id, resolved: sharedPlateFor(f.id, s.id) });
    }
  }
  return out;
}
