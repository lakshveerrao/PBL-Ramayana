...
export async function ping() {
  try {
    ...
    if (res.status === 401 || res.status === 403) {
      // ElevenLabs' error vocabulary distinguishes the two cases, so say which.
      // "invalid_authorization_header" means something IS attached and it is in the
      // wrong header: the credential must use xi-api-key with no prefix, not
      // Authorization. Proven by sending our own xi-api-key alongside - ElevenLabs
      // then answers "Received both".
      const wrongHeader = /invalid_authorization_header|authorization header was invalid/i.test(body);
      return {
        ok: false,
        reason: wrongHeader ? 'a credential is attached, in the wrong header' : 'not authenticated',
        detail: wrongHeader
          ? 'the proxy is attaching an Authorization header. ElevenLabs wants the key in xi-api-key with NO prefix - '
            + 'edit the ElevenLabs API credential on the cloud environment and set Header to xi-api-key, Prefix to none.'
          : explainUnauthorised(res.status, 'ElevenLabs', 'api.elevenlabs.io'),
      };
    }
