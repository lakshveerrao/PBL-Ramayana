...
import { env, assertProxyInUse} from './env.js';
...
export async function ping() {
  assertProxyInUse('api.elevenlabs.io');
  try {
...
// Music. PRODUCTION_ORDERS §6 puts this in Phase D, so it is not built - but it must
// refuse in words rather than as a TypeError from providers.js calling a function that
// was never written.
export async function music() {
  throw new Error(
    'the ElevenLabs music route is not built yet - PRODUCTION_ORDERS puts music in Phase D. '
    + 'direction/music-decision.json records that music is generated; the brief in data/music.json '
    + 'is what it must be generated against, silences included.',
  );
}
