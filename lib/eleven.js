    ...
    if (res.status === 401 || res.status === 403) {
      ...
      if (/invalid_api_key|"Invalid API key"/i.test(body)) {
        return {
          ...
          detail: 'the credential is now reaching ElevenLabs in xi-api-key, so the header is correct. ElevenLabs rejects the key itself: '
                + 'it is expired, revoked, mistyped, or from a different account. Generate a fresh key and re-add the credential '
                + '(delete and add - there is no edit). Nothing about the header needs changing.',
        };
      }
