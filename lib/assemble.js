...
export function motionClipFor(filmId, shotId) {
  ...
  // A FROZEN clip supersedes the raw one. direction/<film>-flame-freeze.json records
  // which shots had a lamp pasted back in from the approved still, and the frozen file
  // sits beside the original rather than replacing it, so the raw clip stays on disk to
  // compare against and the record of what the provider actually returned is intact.
  const frozen = join(ROOT, 'assets', 'motion', filmId, 'frozen', `${shotId}.mp4`);
  const clip = existsSync(frozen) ? frozen : join(ROOT, m.clip);
...
export async function assemble(filmId, lang, { outDir = 'out', stills_only = false } = {}) {
  const t = directedTreatment(filmId);
  const p = plan(filmId);
