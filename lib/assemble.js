import { directedTreatment } from './cut.js';
...
import { chain as gradeChain, grainChain, trimChain } from './grade.js';
...
// The animated clip for a shot, or null. A clip only counts when its record says it was
// made from the still that is CURRENTLY installed: re-render a still and its old clip is
// a picture of a frame that no longer exists, and silently cutting it in would be the
// worst kind of stale.
export function motionClipFor(filmId, shotId) {
  const rec = join(ROOT, 'renders', filmId, 'motion', `${shotId}.json`);
  if (!existsSync(rec)) return null;
  const m = JSON.parse(readFileSync(rec, 'utf8'));
  const clip = join(ROOT, m.clip);
  if (!existsSync(clip)) return null;
  const stillRec = join(ROOT, 'renders', filmId, `${shotId}.json`);
  if (existsSync(stillRec)) {
    const still = JSON.parse(readFileSync(stillRec, 'utf8'));
    if (m.still_sha256 && still.sha256 && m.still_sha256 !== still.sha256) return null;
  }
  return clip;
}

// The ATMOSPHERE pass sits above both. It is made FROM whatever the shot already had -
// a held still, or a generative clip - so it goes stale the same way and by the same
// rule: hash what it was made from, and if that no longer matches, fall through to the
// source rather than cut in a picture of a frame that no longer exists.
export function atmosClipFor(filmId, shotId) {
  const rec = join(ROOT, 'renders', filmId, 'atmos', `${shotId}.json`);
  if (!existsSync(rec)) return null;
  const a = JSON.parse(readFileSync(rec, 'utf8'));
  const clip = join(ROOT, a.clip);
  if (!existsSync(clip)) return null;
  if (a.source_sha256) {
    const src = join(ROOT, a.source);
    if (!existsSync(src)) return null;
    const now = createHash('sha256').update(readFileSync(src)).digest('hex');
    if (now !== a.source_sha256) return null;
  }
  return clip;
}
...
export function plan(filmId) {
  ...
  return {
    film: filmId, fps, totalFrames, cut: t.cut_applied ?? [],
...
export async function assemble(filmId, lang, { outDir = 'out', stills_only = false, atmos_pass = true } = {}) {
  const t = directedTreatment(filmId);
  const p = plan(filmId);
  ...
  //
  // A shot that has been ANIMATED uses its clip instead of its still. The clip was
  // already cut to exactly this shot's frame count and normalised to 30 fps and the
  // frame size by tools/motion_film.js, so it drops straight in where the still was.
  // A shot the graph refuses motion on has no clip and never gets one - 01-15 and
  // 01-16 are frames, by instruction.
  const files = [];
  for (const s of p.shots) {
    let path, clip = false;
    const atmos = atmos_pass ? atmosClipFor(filmId, s.id) : null;
    const motion = motionClipFor(filmId, s.id);
    if (atmos && !stills_only) {
      path = atmos; clip = true;
    } else if (motion && !stills_only) {
      path = motion; clip = true;
    } else if (s.frame.placeholder) {
      path = join(work, `${s.id}.png`);
      makePlaceholder(filmId, s, path);
    } else {
      path = join(ROOT, s.frame.path);
    }
    files.push({ id: s.id, path, frames: s.frames, clip });
  }
  ...
  for (const f of files) {
    ...
    execFileSync('ffmpeg', ['-hide_banner', '-loglevel', 'error',
      ...(f.clip ? ['-i', f.path] : ['-loop', '1', '-framerate', String(p.fps), '-i', f.path]),
      '-frames:v', String(f.frames),
      // A per-shot exposure trim rides HERE, on the shot's own segment, so the film's
      // grade runs on top of it unchanged. direction/grade-trims.json says which.
      '-vf', [`scale=${graphFrame().width}:${graphFrame().height}:force_original_aspect_ratio=decrease:flags=lanczos`,
              ...
              trimChain(filmId, f.id)].filter(Boolean).join(','),
  ...
  writeFileSync(assPath, ass(directedTreatment(filmId), lang), 'utf8');
  ...
  return { film: filmId, lang, file: `${outDir}/${filmId}.${lang}.mp4`, frames: p.totalFrames, fps: p.fps, duration_s: p.duration_s, placeholders: p.placeholders, animated: files.filter((f) => f.clip).map((f) => f.id) };
}
