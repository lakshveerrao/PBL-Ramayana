...
import { malformedKeyHint } from './auth.js';
...
export async function ping() {
  ...
  try {
    ...
    // A rejected key is often a correctly-copied key that was pasted into a field which
    // already held its prefix. Say so before the user goes looking for a new key.
    const hint = classify(e) === 'key rejected' ? malformedKeyHint(key) : null;
    return {
      ok: false,
      reason: 'key rejected',
      detail: hint ? `${hint} (The key itself may be fine.)` : e.message.slice(0, 200),
    };
  }
}
