...
import { env, proxyStatus, assertProxyInUse} from './env.js';
...
async function postOnce(path, body, extraHeaders = {}) {
  ...
  let parsed;
  if (res.ok && streamRequested() && /^\s*(event:|data:)/m.test(text)) {
    parsed = parseImageStream(text);
  } else {
...
export function buildGeneratePayload({ prompt, width = 1080, height = 1920, model = null, quality = null }) {
  ...
  if (streamRequested()) { payload.stream = true; payload.partial_images = 1; }
  return payload;
}
...
// STREAM every image request.
//
// Measured 2026-09-22, and it is not what it looked like. The edits endpoint was
// returning 502 for most calls, and the obvious suspect was request size - the court
// references are 7.3MB of PNG on disk. It is not size:
//
//   plain generation, no references at all    200, ~15s, 3 of 3
//   edit, THREE references at 1.04MB total    502, 30.1-30.5s, 3 of 3
//   edit, ONE reference at 356KB              502, 30.4s, 2 of 2   <- size ruled out
//   edit, three references, quality=low       200, ~15s, 2 of 2    <- duration, not size
//   edit, three references, quality=medium    502, 30.4s, 2 of 2
//   edit, quality=high, stream=true           200, 82-86s, 2 of 2  <- the fix
//
// Every failure lands at ~30.1-30.5s. That is an IDLE timeout on the path, not a limit
// on the request: a high-quality 1152x2048 edit takes 80s+ of silence before the first
// byte, and the connection is cut at 30. A stream emits partial images as it works, so
// the connection is never idle and the whole 86 seconds completes.
//
// So the references are sent EXACTLY as they are - no resizing, no extra compression
// beyond what a 900KB+ file already gets - and the quality is not reduced. Set
// PBL_IMAGE_STREAM=0 to turn it off, which will fail above quality=low.
export function streamRequested() {
  return (process.env.PBL_IMAGE_STREAM ?? '1') === '1';
}

// The stream is server-sent events; the last "completed" event carries the image.
export function parseImageStream(text) {
  let finalB64 = null, usage = null, partials = 0, model = null, done = false;
  for (const line of String(text).split('\n')) {
    ...
    if (/partial_image$/.test(ev.type ?? '')) { partials++; continue; }
    if (/completed$/.test(ev.type ?? '')) {
      done = true;
      finalB64 = ev.b64_json ?? ev.image?.b64_json ?? finalB64;
      usage = ev.usage ?? usage;
      model = ev.model ?? model;
    }
    if (ev.type === 'error' || ev.error) {
      const err = new Error(`openai stream error: ${ev.error?.message ?? JSON.stringify(ev).slice(0, 200)}`);
      err.body = ev;
      throw err;
    }
  }
  if (!done || !finalB64) {
    // A stream that stops early is a failure, not a partial success. Returning the last
    // partial image would put a half-rendered frame into the film.
    const err = new Error(`openai stream ended without a completed image (${partials} partial(s) seen)`);
    err.status = 502;
    throw err;
  }
  return { data: [{ b64_json: finalB64 }], usage, model, _partials: partials };
}
...
export function buildEditForm({ prompt, references, width = 1080, height = 1920, model = null, quality = null }) {
  ...
  if (streamRequested()) { fields.stream = 'true'; fields.partial_images = '1'; }
...
function asBlobParts(ref, i) {
  ...
  // A repo-relative path. Sheets and reference plates are recorded as paths, so this is
  // the ordinary case, not a convenience - and a path is read from inside the repo or
  // not at all, the same rule lib/sheets.js applies to an upload.
  const str = String(ref);
  if (!/^https?:\/\//i.test(str) && /\.(png|jpe?g|webp)$/i.test(str)) {
    const abs = resolve(ROOT, str);
    if (!abs.startsWith(ROOT + sep)) throw new Error(`reference path escapes the repository: ${str}`);
    if (!existsSync(abs)) throw new Error(`reference file does not exist: ${str}`);
    const ext = str.match(/\.([a-z0-9]+)$/i)[1].toLowerCase();
    return {
...
export async function ping() {
  assertProxyInUse('api.openai.com');
  try {
