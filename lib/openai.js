...
// Two ways to authenticate, and in this environment the safer one is already in force:
// the agent proxy attaches an environment API credential after the request leaves the
// session VM. Probed on 2026-09-22 - a deliberately bogus key still returns 200, which
// means the proxy is replacing it. So a key here is neither needed nor trusted, and
// preflight must never report "key valid" on this path.
export function authMode() {
  if (env('OPENAI_AUTH_VIA_PROXY') === '1') return 'proxy';
...
export async function ping() {
  try {
    ...
    if (res.status === 401) {
      ...
      return { ok: false, reason: 'credential rejected', detail: 'HTTP 401 from OpenAI' };
    }
    ...
    return {
      ok: true,
      reason: 'reachable, credential accepted',
      detail: authMode() === 'key'
        ? 'OPENAI_API_KEY is set - but note the agent proxy may be replacing it'
        : 'auth attached by the agent proxy - no key in this container. No image generated.',
