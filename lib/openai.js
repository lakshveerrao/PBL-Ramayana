...
export function buildGeneratePayload({ prompt, width = 1080, height = 1920, model = null, quality = null }) {
  ...
  const payload = { model: m, prompt: String(prompt), size: gen.size, n: 1 };
...
export function buildEditForm({ prompt, references, width = 1080, height = 1920, model = null, quality = null }) {
  ...
  refs.forEach((r, i) => {
    ...
    const type = small.type ?? raw.type;
    const name = small.name;
    // Probed 2026-09-22: multipart takes 'image' or 'image[]' and REFUSES 'images';
    // application/json takes 'images'. The API says so in its own error message.
    form.append('image[]', new Blob([bytes], { type }), name);
  });
  return { form, sent_references: refs.length, size: gen.size };
}

// The proxy answers 502 "upstream request failed" for a large multipart body. The Codex
// portraits are 2.7MB PNGs and a three-reference shot would be 8MB. Measured 2026-09-22:
// the same call with a 140KB JPEG succeeded in 14s.
//
// Re-encoding to JPEG changes the BYTES, not the dimensions - and OpenAI charges input
// image tokens by dimensions, so this costs nothing in fidelity that matters and saves
// the request. Done here rather than at the call sites so no caller can forget.
...
function shrinkIfHuge(bytes, name) {
  ...
  try {
    const { execFileSync } = requireSync();
...
// require() in an ES module: kept in one place, and only for a child_process call that
// has to be synchronous inside a form builder.
function requireSync() {
  return { execFileSync: globalThis.__pblExecFileSync };
}
...
export async function imageFromReference({ prompt, references, negative = null, width = 1080, height = 1920, model = null, quality = null }) {
  ...
  const { form, size } = buildEditForm({ prompt, references, width, height, model, quality });
  const out = normaliseImage(await post('/images/edits', form), model ?? imageModel());
