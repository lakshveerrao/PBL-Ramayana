...
import { endpointCost, openaiImageCost } from './cost.js';
import { acceptsReference as falAcceptsReference } from './endpoints.js';
...
export const image = {
  ...
  // What one image costs on the provider that is ACTUALLY configured, and the name of
  // the endpoint that will serve it.
  //
  // render.js was asking lib/fal.js for the endpoint and lib/cost.js for a fal price
  // while IMAGE_PROVIDER was openai. Every M1 estimate came back as fal-ai/flux/dev at
  // $0.025 - and the same call feeds recordSpend, so the ledger would have recorded fal
  // prices for OpenAI spend. An estimate that names the wrong endpoint is not a rounding
  // error; it is a number about a different thing.
  cost({ references = 0, quality = null } = {}) {
    const p = configured().image;
    if (p === 'openai') {
      const model = openai.imageModel();
      const c = openaiImageCost({ model, references, quality: quality ?? process.env.PBL_IMAGE_QUALITY ?? 'auto' });
      return { usd: c.usd, endpoint: model, known: c.measured === true, reference_conditioned: true, note: c.note };
    }
    if (p === 'fal') {
      const endpoint = fal.imageEndpoint();
      const c = endpointCost(endpoint);
      return { usd: c.usd, endpoint, known: c.known, reference_conditioned: falAcceptsReference(endpoint), note: c.note ?? null };
    }
    return { usd: 0, endpoint: p, known: false, reference_conditioned: false, note: `no pricing for image provider "${p}"` };
  },
